...
uint8_t alertMinRepeats(DetectionType t) { return ruleForConst(t).minRepeats; }
...
const char* alertRuleSummary(DetectionType t) {
    ...
    const AlertRule& r = ruleForConst(t);
    snprintf(b, sizeof b, "%ux %s >", (unsigned)r.minRepeats, alertRuleConfidenceLabel(t));
    return b;
}
void toggleAlertEnabled(DetectionType t) {
    AlertRule& r = ruleFor(t); r.flags ^= AR_ALERT; saveAlertRules();
}
...
void cycleAlertMinRepeats(DetectionType t) {
    AlertRule& r = ruleFor(t);
    r.minRepeats = (r.minRepeats == 1) ? 2 : (r.minRepeats == 2 ? 3 : (r.minRepeats == 3 ? 5 : 1));
    saveAlertRules();
}
void cycleAlertCooldown(DetectionType t) {
    AlertRule& r = ruleFor(t);
    r.cooldownIx = (uint8_t)((r.cooldownIx + 1) % ALERT_COOLDOWN_N);
    saveAlertRules();
}
