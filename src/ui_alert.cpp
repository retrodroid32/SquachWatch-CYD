...
void uiAlertTick(TFT_eSPI& t, uint32_t now, const DetectionEngine& eng,
    ...
    // Identity/type stay from the alert snapshot, but signal history keeps
    // moving while the card is open.
    ...
    // number is for the log; the bar is what reads from across a room.
    {
        ...
        // Last ~15 seconds as a tiny sparkline on top of the current-strength
        // fill. Vertical scale is the same honest -90..-40 dBm scale.
    ...
    if (trend == RssiTrend::UNKNOWN) {
        snprintf(info, sizeof(info), "%d dBm   CH %u   x%u",
                 signalDet.rssi, s_last.channel, (unsigned)signalDet.repeats);
    } else {
        snprintf(info, sizeof(info), "%d dBm  %s", signalDet.rssi, rssiTrendName(trend));
    }
