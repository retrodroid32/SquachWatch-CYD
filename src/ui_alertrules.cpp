...
static void drawDetailRow(TFT_eSPI& t, int y, const char* label,
...
void uiAlertRulesTick(TFT_eSPI& t, uint32_t now) {
    ...
    Theme::drawTitleBar(t, g_detail ? detectionTypeDisplayName(g_selected)
                                    : ">> ALERT RULES <<");
    ...
    if (!g_detail) {
        ...
        if (n > visible)
            Theme::drawScrollbar(t, w - 3, top, bottom - top, n, visible, g_scroll);
        return;
    }

    Theme::drawListHeading(t, "RULE", Theme::colorFor(g_selected));
    int y = bodyTop();
    drawDetailRow(t, y, "FULL ALERT",
                  onOff(Settings::alertEnabled(g_selected)), Theme::AMBER); y += ROW_H;
    drawDetailRow(t, y, "MIN CONF",
                  Settings::alertRuleConfidenceLabel(g_selected), Theme::CYAN); y += ROW_H;

    char rep[12];
    snprintf(rep, sizeof rep, "%u HITS",
             (unsigned)Settings::alertMinRepeats(g_selected));
    drawDetailRow(t, y, "MIN REPEATS", rep, Theme::CYAN); y += ROW_H;
    drawDetailRow(t, y, "COOLDOWN",
                  Settings::alertRuleCooldownLabel(g_selected), Theme::VAPOR_PURPLE); y += ROW_H;
    drawDetailRow(t, y, "WAKE SCREEN",
                  onOff(Settings::alertWakeScreen(g_selected)), Theme::GREEN);

    t.setTextSize(1);
    t.setTextColor(Theme::W95_SHADOW, Theme::BG);
    const char* hint = Settings::alertEnabled(g_selected)
                     ? "Blocked alerts still stay in the LOG."
                     : "LOG ONLY: counted and recorded, no alert.";
    t.setCursor(7, bodyBottom(w, h) - t.fontHeight() - 5);
    t.print(hint);
}

AlertRuleHit uiAlertRulesHitTest(TFT_eSPI& t, int x, int y,
    ...
    if (!g_detail) {
        const int n = (int)DetectionType::COUNT - 1;
        uiClampScroll(g_scroll, n, bottom - top, ROW_H);
        const int row = g_scroll + (y - top) / ROW_H;
